...
#include "old/es-braille-cells.h"
#include <stdio.h>
 ...
 * @param el string a imprimr
 */
void display_braille_str(char buffer[]) {
  for (char curr = *buffer; curr != '\0'; curr = *++buffer) {
...
BrailleCell get_char(char character, char modifier) {
  ...
  if (modifier == '_' && character == 'u') /* dieresis */
    return braille_char[UMLAUT_U];

  if (modifier == '~' && character == 'n') /* eñes */
    return braille_char[ACCENT_N];

  if (modifier == '^') { /* acentos */
    switch (character) {
    case 'a':
      return braille_char[ACCENT_A];
      break;

    case 'e':
      return braille_char[ACCENT_E];
      break;

    case 'i':
      return braille_char[ACCENT_I];
      break;

    case 'o':
      return braille_char[ACCENT_O];
      break;

    case 'u':
      return braille_char[ACCENT_U];
      break;
    }
  }
...
int str_to_braille(char input[], char output[]) {
  ...
  for (char curr = *input; curr != '\0'; curr = *++input) {
    ...
    if (is_capital(curr))
      output[pos++] = dots(braille_char[UPCASE_PREFIX]);

    if (is_numeric(curr) && !is_numeric(prev))
      output[pos++] = dots(braille_char[NUMBER_PREFIX]);

    if (is_alphabetic(curr) && is_numeric(prev))
      output[pos++] = dots(braille_char[NUMBER_INTERRUPTOR]);
